...
import * as Aliases from '../data/aliases';
import * as Conditions from '../data/conditions';
import * as FormatsData from '../data/formats-data';
import * as Items from '../data/items';
import * as Learnsets from '../data/learnsets';
import * as Moves from '../data/moves';
import * as Natures from '../data/natures';
import * as Pokedex from '../data/pokedex';
import * as Rulesets from '../data/rulesets';
import * as Scripts from '../data/scripts';
import * as TypeChart from '../data/typechart';
import * as PokemonGoData from '../data/pokemongo';

import * as TextAbilities from '../data/text/abilities';
import * as TextDefault from '../data/text/default';
import * as TextItems from '../data/text/items';
import * as TextMoves from '../data/text/moves';
import * as TextPokedex from '../data/text/pokedex';

const DATA_FILES_IMPORT = {
	Abilities,
	Aliases,
	Conditions,
	FormatsData,
...
const TEXT_FILES_IMPORT = {
	abilities: TextAbilities,
	default: TextDefault,
	items: TextItems,
	moves: TextMoves,
	pokedex: TextPokedex,
...
export class ModdedDex {
	...
	loadDataFile(basePath: string, dataType: DataType): AnyObject | void {
		if (dataType in DATA_FILES_IMPORT) {
			return (DATA_FILES_IMPORT[dataType as keyof typeof DATA_FILES_IMPORT] as any)[dataType];
		}
	...
	loadTextFile(
		...
		if (name in TEXT_FILES_IMPORT) {
			return (TEXT_FILES_IMPORT[name as keyof typeof TEXT_FILES_IMPORT] as any)[exportName];
		}
		return {};
	}

	includeMods(): this {
		// Cloudflare Workers: No filesystem access, so we only support base mod for now.
	...
	loadAliases(): NonNullable<ModdedDex['aliases']> {
		...
		const exported = DATA_FILES_IMPORT.Aliases;
