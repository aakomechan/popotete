...
import { Formats as FormatsList } from '../config/formats';
...
export class DexFormats {
	...
	load(): this {
		...
		// 	}
		// }
		let Formats: AnyObject[] = FormatsList;
		...
		for (const [i, format] of Formats.entries()) {
			...
			if (!this.dex.dexes[format.mod]) continue; // throw new Error(`Format "${format.name}" requires nonexistent mod: '${format.mod}'`);
